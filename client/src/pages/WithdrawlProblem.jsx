...
import { getwithdrawl } from "../utils/slice";
...
function WithdrawalProblem() {
  ...
  const rechargDetails = useSelector((store) => store?.slice?.withdrawl);

  useEffect(() => {
    if (auth) {
      dispatch(getwithdrawl(auth));
    }
  }, [auth, dispatch]);
      ...
      <div className="space-y-4 p-2">
        {rechargDetails?.data?.length > 0 ? (
          rechargDetails.data.map((item, i) => (
            <div key={i} className="bg-white rounded p-2 shadow-sm space-y-2">
              {/* Top */}
              ...
              <div className="flex justify-between text-sm sm:text-base items-center">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <IoCopyOutline
                    ...
                    onClick={() => copyToClipboard(item?.id_order)}
                  />
                </div>
              </div>
              ...
              <div className="pt-2">
                <button
                  onClick={() => navigate("/withdrawlproblem")}
